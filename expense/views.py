...
def home_app(request):
    ...
    return render (request, 'expense/home1.html')
...
class ExpenseChartView(TemplateView):
    ...
    template_name = 'expense/chart.html'
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['qs'] = Expense.objects.all()
        return context

class ExpensePieView(TemplateView):

    template_name = 'expense/chart1.html'
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['qs'] = Expense.objects.all()
        return context
...
def sum_exp(request):
    try:
        expense_total = Expense.objects.filter(author=request.user).aggregate(expenses=Sum('amount'))
    except TypeError:
        print('No data')
    ...
    return render(request, 'expense/sum.html', data )
